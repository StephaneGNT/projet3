...
class CheesecakeSizeSelection extends Component {
  constructor(props) {
    super(props);
    this.state = { 
      size: 0,
  ...
  renderCakeSizeChoice = () => {
    ...
    for (let i = 0; i < storys.length; i += 1) {
      ...
      let disabledPlus = this.state.story > 0;
      let disabledMinus = this.state.chosenStory.indexOf(storys[i]) === -1;
      render.push(
        <Container className="cakeSize">
          <Row>
            ...
            <Col sm="3" className="buttonDisplay">
              <Button disabled={disabledPlus} onClick={() => this.addPieces(storys[i])}>+</Button>
              <Button disabled={disabledMinus} onClick={() => this.removePieces(storys[i])}>-</Button>
            </Col>
          </Row>
        </Container>,
      );
  ...
  render() {
    return (
      <Container style={{ minWidth: '100%' }}>
        <Label className="labels-perso mt-3">Choisissez la taille de votre cheesecake</Label>
        <Row>
...
export default connect(null, mapDispatchToProps)(CheesecakeSizeSelection);
