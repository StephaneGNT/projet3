...
// Enregistrement de la commande finale - OK
...
// Récupération de l'ensemble des orders - OK
order.get(`/orders/all`, (req, res) => {
  connection.query('SELECT * FROM final_orders', (err, results) => {
    if (err) res.status(500).send("Erreur lors de la commande");
    else res.status(200).send(results);
  });
});
